...
export async function POST(request: Request) {
  ...
  const priceId = process.env.STRIPE_ADVISOR_PRICE_ID;
  const appUrl = process.env.NEXT_PUBLIC_APP_URL || "https://exhaledebt.com";

  if (!stripeKey || !priceId) {
    return Response.json({ error: "Advisor checkout is not configured." }, { status: 503 });
  }
  ...
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      metadata: { firebaseUid: uid, type: "advisor_lifetime" },
    });

    return Response.json({ url: session.url });
  } catch (err) {
    console.error("Advisor checkout error:", err instanceof Error ? err.message : err);
